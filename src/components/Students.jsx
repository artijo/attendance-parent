...
export default function Students() {
  ...
  useEffect(() => {
    // ดึง userId จาก sessionStorage หรือ LINE Login
  ...
  const fetchStudents = async () => {
    try {
      ...
      setError(
        `ไม่สามารถดึงข้อมูลนักเรียนได้ (${error.message}) กรุณาลองใหม่อีกครั้ง`,
      );
      setLoading(false);
    }
  };
  ...
  const cancelNotification = async (studentId) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `ไม่สามารถยกเลิกการรับการแจ้งเตือนได้ (${response.status})`,
        );
      }
      ...
      setStudents((prevStudents) =>
        prevStudents.filter((s) => s.student.stdId !== studentId),
      );
      ...
      setError(
        error.message ||
          "ไม่สามารถยกเลิกการรับการแจ้งเตือนได้ กรุณาลองใหม่อีกครั้ง",
      );
    } finally {
  ...
  const confirmCancelNotification = (student) => {
    const studentName = `${formatTitle(student.student.title)}${student.student.fName} ${subLastname(student.student.lName)}`;
    const studentId = student.student.stdId;

    if (
      window.confirm(
        `คุณต้องการยกเลิกการรับการแจ้งเตือนสำหรับ ${studentName} ใช่หรือไม่?`,
      )
    ) {
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="hidden sm:block overflow-x-auto">
              <table className="min-w-full divide-y divide-line">
                <thead>
                  <tr>
                    <th className="px-4 py-3 bg-gray-50 text-left text-xs font-medium text-text-alt uppercase tracking-wider">
                      ลำดับ
                    </th>
                    <th className="px-4 py-3 bg-gray-50 text-left text-xs font-medium text-text-alt uppercase tracking-wider">
                      ชื่อ-นามสกุล
                    ...
                    <th className="px-4 py-3 bg-gray-50 text-center text-xs font-medium text-text-alt uppercase tracking-wider">
                      การแจ้งเตือน
                    </th>
                  </tr>
                </thead>
                    ...
                    <tr key={student.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="text-sm font-medium text-text">
                          ...
                          {subLastname(student.student.lName)}
                        </div>
                      </td>
                      {/* <td className="px-4 py-3 whitespace-nowrap">
            ...
            <div className="sm:hidden space-y-3">
              {students.map((student, index) => (
                <div
                  key={index}
                  className="bg-white rounded-md border border-line-alt p-3 shadow-sm"
                >
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className="text-xs bg-blue-100 text-primary px-2 py-1 rounded-full font-medium">
                      ม.
                      {student.student.classroomMembers[0].classroom.classLevel}
                      /{student.student.classroomMembers[0].classroom.classRoom}
                    </span>
                  </div>
                  <div className="flex justify-between items-center text-xs">
                    ...
                    >
                      {cancelingStudent === student.student.stdId ? (
                        <>
                          <svg
                            className="animate-spin -ml-0.5 mr-1 h-3 w-3 text-red-500"
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                            viewBox="0 0 24 24"
                          >
          ...
          <div className="py-8 text-center bg-gray-50 rounded-lg border border-dashed border-gray-300">
            ...
            <h3 className="mt-2 text-sm font-medium text-text">
              ยังไม่มีนักเรียนในความดูแลของท่าน
            </h3>
            <p className="mt-1 text-sm text-text-alt mb-4">
              คุณสามารถเพิ่มนักเรียนในความดูแลได้ทันที
            </p>
            <button
              onClick={goToAddStudent}
