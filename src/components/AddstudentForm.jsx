...
function AddStudentForm() {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          setError(
            "ไม่พบข้อมูลนักเรียนที่ระบุ กรุณาตรวจสอบรหัสนักเรียนอีกครั้ง",
          );
        } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await response.json();
      setSuccess(
        `เพิ่มนักเรียน ${formatTitle(studentData.title)}${studentData.fName} ${studentData.lName} เรียบร้อยแล้ว`,
      );
      setStudentId("");
      setStudentData(null);
    ...
    <div className="bg-white p-4 sm:p-6 rounded-lg shadow-sm">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label
            htmlFor="studentId"
            className="block text-sm font-medium text-text-alt mb-1"
          >
            รหัสนักเรียน
          </label>
          <div className="relative rounded-md shadow-sm">
            ...
            <input
              ...
              value={studentId}
              onChange={(e) =>
                setStudentId(e.target.value.replace(/\D/g, "").substring(0, 5))
              }
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="bg-white p-3 rounded-md shadow-sm">
                ...
                <div className="font-medium">
                  {formatTitle(studentData.title)}
                  {studentData.fName} {subLastname(studentData.lName)}
                </div>
              </div>
              ...
              {/* {studentData.classroomMembers &&
                studentData.classroomMembers.length > 0 && (
                  <>
                    <div className="bg-white p-3 rounded-md shadow-sm">
                      <div className="text-sm text-gray-500">ชั้นเรียน</div>
                      <div className="font-medium">
                        {formatClass(studentData.classroomMembers[0].classroom)}
                      </div>
                    </div>
                    ...
                    <div className="bg-white p-3 rounded-md shadow-sm">
                      ...
                      <div className="font-medium">
                        {studentData.classroomMembers[0].stdNo || "-"}
                      </div>
                    </div>
                    ...
                    <div className="bg-white p-3 rounded-md shadow-sm col-span-1 sm:col-span-2">
                      ...
                      <div
                        className={`font-medium text-lg ${getBehaviourScoreClass(studentData.classroomMembers[0].behaviourScore)}`}
                      >
                        {studentData.classroomMembers[0].behaviourScore || "-"}{" "}
                        คะแนน
                      </div>
                    </div>
                  </>
